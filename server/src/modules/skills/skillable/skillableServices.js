...
export const getSkillablesByIds = async (ids) => {
  try {
    ...
    for (const skill of skills) {
      ...
      const data =
        skill.skillableType === "experience"
          ? await getExperienceInfoByIdSvc(skill.skillableId)
          : skill.skillableType === "project"
          ? await getProjectInfoByIdSvc(skill.skillableId)
          : null;
      coso[skill.skill.name].items.push({
        id: skill.skillableId,
        type: skill.skillableType,
        data,
      });
    }

    return Object.entries(coso).map(([skillName, skillData]) => [
      skillName,
      skillData.items,
      skillData.id,
    ]);
...
export const createSkillables = async (
  skillableId,
  skills,
  skillableType,
  t
  ...
  try {
    if (!Array.isArray(skills)) {
      return Skillable.create(
        { skillId: skills, skillableId, skillableType },
        { transaction: t }
      );
    }
    if (skills.length > 0) {
      await Promise.all(
        skills.map((skill) =>
          Skillable.create(
            { skillId: skill, skillableId, skillableType },
            { transaction: t }
          )
        )
      );
...
export const deleteSkillables = async (skillableId, skills, t) => {
  try {
    if (!Array.isArray(skills)) {
      ...
      if (skillable) {
        return Skillable.destroy(
          { where: { skillId: skills, skillableId } },
          { transaction: t }
        );
      }
    }
    if (skills.length > 0) {
      await Promise.all(
        skills.map(async (skill) => {
          ...
          if (skillable) {
            await Skillable.destroy(
              { where: { skillId: skill, skillableId } },
              { transaction: t }
            );
          }
        })
      ...
      return Skillable.destroy({ where: { skillableId }, transaction: t });
    }
  } catch (error) {
    throw error;
  }
