...
export const createExperience = async (experience, skills, username) => {
  try {
    ...
    formData.append(
      "startDate",
      `${experience.startDateMonth}/01/${experience.startDateYear}`
    );
    formData.append(
      "endDate",
      experience.endDateMonth !== ""
        ? `${experience.endDateMonth}/01/${experience.endDateYear}`
        : null
    );
    formData.append("isRecruited", experience.isRecruited);
...
export const updateExperience = async (
  ...
  const formattedExperience = {
    ...
    endDate:
      experience.endDateMonth !== "null"
        ? `01/${experience.endDateMonth}/${experience.endDateYear}`
        : null,
    skills: skills,
  };
  ...
  if (formattedExperience.endDate !== null) {
    if (
      Date.parse(formattedExperience.endDate) <
      Date.parse(formattedExperience.startDate)
    ) {
      throw new Error(
        "La fecha de fin no puede ser anterior a la fecha de inicio"
      );
  ...
  const res = await axios.patch(
    `${BASE_URL}/experience/${username}/${experienceId}`,
    {
